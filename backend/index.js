...
// ------------------ Development-Only: Seed Routes ------------------
if (process.env.NODE_ENV !== "production") {
  app.get("/addHoldings", async (req, res) => {
    const tempHoldings = [/* your holdings data here */];
    try {
      await HoldingsModel.insertMany(tempHoldings);
      res.send("Holdings seeded successfully.");
    } catch (err) {
      console.error("Error seeding holdings:", err);
      res.status(500).send("Failed to seed holdings.");
    }
  });

  app.get("/addPositions", async (req, res) => {
    const tempPositions = [/* your positions data here */];
    try {
      ...
      res.status(500).send("Failed to seed positions.");
    }
  });
}
